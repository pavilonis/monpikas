...
@Repository
public class ProductRepository implements EntityRepository<Product, Long, ProductFilter> {
   ...
   private static final String FROM_WHERE_BLOCK = "" +
         ...
         "  AND (:productGroupId IS NULL OR pg.id = :productGroupId) ";
   ...
   private Product create(Map<String, Object> args) {
      ...
      jdbcNamed.update("" +
                  "INSERT INTO Product (name, measureUnit, unitWeight, productGroup_id, dateCreated) " +
                  "VALUES (:name, :measureUnit, :unitWeight, :productGroupId, :dateCreated)",
   ...
   public List<Product> load() {
      return load(new ProductFilter(null));
   }

   @Override
   public List<Product> load(ProductFilter filter) {
   ...
   public Optional<Product> find(Long id) {
      List<Product> result = load(new ProductFilter(id));
   ...
   @Override
   public Optional<SizeConsumingBackendDataProvider<Product, ProductFilter>> lazyDataProvider(ProductFilter filter) {
      SizeConsumingBackendDataProvider<Product, ProductFilter> provider = new SizeConsumingBackendDataProvider<Product, ProductFilter>() {
         @Override
         protected Stream<Product> fetchFromBackEnd(Query<Product, ProductFilter> query) {
            ProductFilter updatedFilter = filter
   ...
   private Map<String, Object> composeArgs(ProductFilter filter) {
      Map<String, Object> args = new HashMap<>();
      args.put("id", filter.getId());
      args.put("name", QueryUtils.likeArg(filter.getText()));
      args.put("productGroupId", filter.getProductGroup() == null ? null : filter.getProductGroup().getId());
